...
function addTableRow(publication, tbody) {
  ...
  // Lead image.
  text += "<td style='width: 25%; padding-right: 1em; padding-bottom: 1em'><a href='" + publication.venue.link + "' target='_blank'><img src='images/publications/" + publication.venue.image + "' style='width: 100%' alt=" + publication.venue.title + "'></a></td>";

  // Authors.
  text += "<td style='width: 75%; vertical-align: top; padding-bottom: 1em; line-height: 1.4em'>";
  for (let i = 0; i < publication.authors.length; i++) {
    ...
    var name_check = publication.authors[i].toLowerCase();
      if (name_check == "l. schmid" || name_check == "lukas schmid" || name_check == "schmid, l." || name_check == "schmid, lukas") {
        text += "<strong>" + publication.authors[i] + "</strong>";
      } else {
  ...
  // Title and reference.
  text += "<br><span style='font-weight: bold; color: rgb(80, 100, 200);'>" + publication.title + "</span><br>" + publication.venue.name + ", " + publication.pages + ", " + publication.year + ".<br>";
